...
CString CoinTigerAPI::Get_Times()
{
	SYSTEMTIME st = { 0 };
...
CString CoinTigerAPI::Get_Sendmsg(CString* intmp, int mun, CString Sub,CString GetOfPost)
{
	//GetOfPost = "GET"/"POST";
	CString Url = "api.cointiger.pro";
...
CString CoinTigerAPI::find_str(CString inCStr, const char* exptxt)
{
...
void CoinTigerAPI::writeLog(CString msg, CString fxname)
{
	ofstream outfile;
	...
	{
		outfile << Get_Times() << "\t" << "CoinTigerAPI::" << fxname << "\t" << msg << endl;
		outfile.close();
	}
}
...
/************************************************************************/
CString CoinTigerAPI::Get_balance_matchresults_order(CString sub,CString order_id)
{
	CString phat;
	if (sub=="余额")
	{
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt                           */
/************************************************************************/
BOOL CoinTigerAPI::Get_currencys()
{
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
		"SignatureVersion", "=2",
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt										 */
/*************************************************************************************/
BOOL CoinTigerAPI::Get_market_depth(CString symbol,CString type,int buy_depth,int sell_depth)
{
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt                           */
/************************************************************************/
BOOL CoinTigerAPI::Get_market_history(CString symbol, int mun)
{
	CString strtemp;
...
/************************************************************************/
CString CoinTigerAPI::Post_place(CString symbol,CString price, CString mun,CString source,CString type)
{
	//↓↓↓↓↓↓↓↓↓↓↓↓↓↓精度调整↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt                           */
/************************************************************************/
BOOL CoinTigerAPI::Post_submitcancel(CString order_id)
{
	CString sendmsg;
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
...
/************************************************************************/
//CString CoinTigerAPI::Get_ordermsg(CString order_id)
//{
//	CString sendmsg;
//	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
//		"SignatureMethod", "=HmacSHA256",
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt                           */
/************************************************************************/
BOOL CoinTigerAPI::Post_batchcancel(CString *order_id, int mun)
{
	CString sendmsg;
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
	...
	string reponse_data = PostRequest("api.cointiger.pro", sendmsg, senddata);
...
/************************************************************************/
BOOL CoinTigerAPI::Get_margin_balance(CString symbol)
{
...
/************************************************************************/
CString CoinTigerAPI::Post_transfer_inout_apply(CString phat, CString symbol, CString currency, CString amount)//借贷订单
{
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
		"SignatureVersion", "=2",
...
/*返回值：成功TRUE 失败FALSE并写入err_log.txt                           */
/************************************************************************/
BOOL CoinTigerAPI::Post_margin_repay(CString order_id,CString amount)
{
	CString sendmsg;
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
	...
	sendmsg.Delete(0, sendmsg.Find("/v1", 0));
	CString msg = PostRequest("api.cointiger.pro", sendmsg, "{\"amount\":\""+ amount +"\"}");
	if (-1 == msg.Find("error"))//发送成功
	{
		/*msg.Delete(0, msg.Find("data", 0));
		CString sstr = find_str(msg, "[0-9]+");*/
...
/*返回值：成功/失败 返回原始数据 失败写入err_log.txt                    */
/************************************************************************/
CString CoinTigerAPI::Get_路径无参数通用(CString phat, CString *instr, int mun)
{
	int j = 8;
	CString Q[64] = {"0"};
...
/************************************************************************/
CString CoinTigerAPI::Post_自写路径通用(CString phat, CString *instr, int mun)
{
	CString sendmsg;
	CString Q[8] = { "AccessKeyId", "=" + Access_Key,
		"SignatureMethod", "=HmacSHA256",
	...
	CString msg = PostRequest("api.cointiger.pro", sendmsg, "");
	if (-1 == msg.Find("error"))//发送成功
	{
		/*msg.Delete(0, msg.Find("data", 0));
		CString sstr = find_str(msg, "[0-9]+");*/
